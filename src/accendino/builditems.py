...
class BuildArtifact(DepsBuildArtifact):
    ...
    def prepare(self, config) -> bool:
        ...
        dumpsEnvEqual = dump2 is not None
        if dumpsEnvEqual:
...
class CMakeBuildArtifact(BuildArtifact):
    ...
    def prepare(self, config) -> bool:
        ...
        if config.crossCompilation:
            cmake_cmd.append(f'-DCMAKE_TOOLCHAIN_FILE={config.getCrossPlatformFile("cmake", config.distribId, config.targetDistrib, config.targetArch)}')
